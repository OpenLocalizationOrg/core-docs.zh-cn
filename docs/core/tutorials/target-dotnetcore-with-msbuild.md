...
ms.openlocfilehash: 7ad36e97d1389e5dda7d7bff89a3e2b2ccdff9d1
...
We expect the first version of the .NET Core tools that use MSBuild to ship along with the next version of Visual Studio.  However, it is possible to use MSBuild for .NET Core projects today, and this page shows how.
...
6. Open the `project.json` file in your project, and make the following changes:
    ...
    - Add the below `imports` definition inside the `netstandard1.6` framework definition.  This will allow your project to reference .NET Core compatible NuGet packages that haven't been updated to target .NET Standard
...
7. In Solution Explorer, right click on the project, choose **Unload Project**, then right click again and choose **Edit _MyProj.csproj_**, and make the following changes
